...
from onedrive_fuse import config, eventq, localonly, metrics, common, commonfunc
from onedrive_fuse.remote import api, attr, chown, getxattr, mkdir, mkdir, readdir, readlink, rmdir
from onedrive_fuse import log

from fuse import FUSE, FuseOSError, Operations

from onedrive_fuse import data, directories
from onedrive_fuse import metadata
from onedrive_fuse import db
from onedrive_fuse.log import logger
from onedrive_fuse.threads import heartbeat, refreshcache, rpc, tcreate, tdelete, tdownload, tupload,refresh
from onedrive_fuse.remote import truncate, chmod, utime, create, remove, rename, symlink
...
class FuseOps(Operations):
    ...
    def __init__(self, args): 
        ...
        try:
            api.onedrive.authenticate()
        except Exception as e:
            raisedBy = commonfunc.exceptionRaisedBy(e)
            logger.error(f'Authentication failed: {raisedBy}')            
            return          
        ...
        try:             
            ...
            print('To unmount filesystem: onedrive-fuse unmount {}'.format(args.mountpoint))
