...
from onedrive_fuse import color, common

_ONEDRIVE_FUSE     = 'onedrive_fuse'
_ONEDRIVE_FUSE_METRICS = 'onedrive_fuse_metrics'
...
class TidFilter(logging.Filter):
    def filter(self, record):
        ...
        if hasattr(common.threadLocal, 'operation') and (record.name == _ONEDRIVE_FUSE):
            status = color.red('OFFLINE ') if common.offline else color.green('ONLINE ')
            record.name = status + color.cyan(common.threadLocal.operation+f" path={common.threadLocal.path}:")
        else:
...
class PathFilter(logging.Filter):
    def filter(self, _record): 
        ...
        for path in common.threadLocal.path:
            if isinstance(path, str) and path == common.pathfilter:
                return True       
        return False
...
class Log:
    ...
    def setupConfig(self, debug: bool, verbose: bool):    
        ...
        # error logging
        for name in [_ONEDRIVE_FUSE, _FUSE, _PARAMIKO, _PARAMIKO_SFTP]:
        ...
        metricsLogger = logging.getLogger(_ONEDRIVE_FUSE_METRICS)
        metricsLogger.addHandler(metricsHandler)
        metricsLogger.setLevel(logging.INFO) 

logger = logging.getLogger(_ONEDRIVE_FUSE)
metricsLogger = logging.getLogger(_ONEDRIVE_FUSE_METRICS)

for name in [_ONEDRIVE_FUSE, _ONEDRIVE_FUSE_METRICS, _PARAMIKO, _PARAMIKO_SFTP]:
    l = logging.getLogger(name)
    l.addFilter(TidFilter())
    l.addFilter(PathFilter()) if name != _ONEDRIVE_FUSE_METRICS else None
