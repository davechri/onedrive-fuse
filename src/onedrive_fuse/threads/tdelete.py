...
from onedrive_fuse import common, commonfunc, metrics, lock, stats
from onedrive_fuse.log import logger

from onedrive_fuse.remote import api, modes
from onedrive_fuse.remote.readdir import FuseOSError
...
class Delete:   
    ...
    def enqueue(self, path: str, localId: str,onedriveId: int):        
        self.queue.put((path, localId, onedriveId))

    def worker(self, number: int):        
        common.threadLocal.operation = 'tdelete_%d' % number
        common.threadLocal.path = None
        while not self.stopped:            
            (path, localId, onedriveId) = self.queue.get()            
                ...
                try:
                    ...
                    for timeout in commonfunc.apiTimeoutRange():
                        try: 
                            ...
                            api.onedrive.delete(path)
                            modes.deleteMode(localId)
                            break
