...
from onedrive_fuse import common, commonfunc, db, data, directories, eventq, filesystem, mem, metrics, metadata, localonly
from onedrive_fuse.remote import attr
from onedrive_fuse.log import logger
from onedrive_fuse.threads import tdelete
from onedrive_fuse.threads import tcreate, tdownload, tupload
from onedrive_fuse.stats import calledStats, calledStatsSnapshot, remoteStats, remoteStatsSnapshot
    ...
    def directories(self) -> str:        
        ...
        try:
            for dir in directories.store.getAllDirectories():                
                ...
                output.append(f'ByPath: path={path} name={dir.name} onedriveId={dir.onedriveId} local_id={dir.localId} local_parent_id={dir.localParentId}')
                ...
                if dirByLocalId is not dir:
                    output.append(f'ByLocalId: path={dirByLocalId.path} name={dirByLocalId.name} onedriveId={dirByLocalId.onedriveId} local_id={dirByLocalId.localId} local_parent_id={dirByLocalId.localParentId}')
