...
from onedrive_fuse import common, commonfunc, data, db, gitignore, lock, metadata, metrics, stats
            ...
            with lock.get(path):
                ...
                try:
                    ...
                    with open(localPath, 'rb') as f:                    
                        api.onedrive.upload(path, localPath)
                        d['st_size'] = fileSize
                    ...
                    if isinstance(e, TimeoutError):
                        ...
                        logger.error(f"<-- uploadThread: exception writing file {path} {localPath}: {raisedBy}")
                finally:
                    self.activeThreadCount -= 1
