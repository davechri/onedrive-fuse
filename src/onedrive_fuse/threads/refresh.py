...
from onedrive_fuse import common, eventq, commonfunc, metrics
from onedrive_fuse.log import logger
from onedrive_fuse.threads import tcreate, tdelete,refreshcache
...
class Refresh:
    ...
    def trigger(self, j: dict[str, any]):
        """Triggers the refresh thread to start the refresh process by setting the event flag.""" 
        logger.info('refresh.trigger: triggering refresh due to heartbeat changes')
        self.delta = j
            ...
            try:
                ...
                if elapsed > common.updateinterval:  
                    ...
                    if tdelete.manager.activeThreadCount > 0 or tdelete.manager.queue.qsize() > 0 or tcreate.manager.activeThreadCount > 0 or tcreate.manager.queue.qsize() > 0:
                        ...
                        refreshcache.refreshDelta(self.delta) # Refresh all files                
