...
from onedrive_fuse import common, commonfunc, data, metadata, eventq, metrics
...
def execute(oldpath: str, newpath: str, runAsync: bool=True) -> None: 
    ...
    if dNew is not None:
        if dNew['st_mode'] &  stat.S_IFDIR:
            if dOld['local_only']:
                ...
                eventq.queue.enqueueDirEvent(newpath, dNew['local_id'], dNew['onedrive_id'])
            else:
            ...
            if dNew['local_only']:
                ...
                if dNew['st_mode'] & stat.S_IFLNK == stat.S_IFLNK:
                    ...
                    eventq.queue.enqueueSymlinkEvent(newpath, dNew['local_id'], dNew['onedrive_id'])
                else:
                    metrics.counts.incr('rename_delete_existing_file_enqueue')
                    eventq.queue.enqueueFileEvent(newpath, dNew['local_id'], dNew['onedrive_id'])
            else:
    ...
    if not runAsync:             
        ...
        eventq.queue.enqueueRenameEvent(oldpath, newpath, dOld['local_id'], dOld['onedrive_id'])
    else:
...
def doRename(oldpath: str, newpath: str, d: dict[str,any]) -> None:
    for timeout in commonfunc.apiTimeoutRange():
        try:     
            ...
            api.onedrive.patch(oldpath, {'name': os.path.basename(newpath)})
            break
